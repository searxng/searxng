...
from searxng.engines import wikidata, set_loggers
from searxng.utils import extract_text, match_language
from searxng.locales import LOCALE_NAMES
from searxng import searxng_dir
from searxng.utils import gen_useragent
import searxng.search
import searxng.network
...
def get_wikipedia_summary(lang, pageid):
    ...
    searxng.engines.engines['wikipedia'].request(pageid, params)
    try:
        response = searxng.network.get(params['url'], headers=params['headers'], timeout=10)
...
def get_website_description(url, lang1, lang2=None):
    ...
    try:
        response = searxng.network.get(url, headers=headers, timeout=10)
        response.raise_for_status()
    except Exception:
        return (None, None)
...
def initialize():
    ...
    wikipedia_engine = searxng.engines.engines['wikipedia']
    WIKIPEDIA_LANGUAGES = {
    ...
    for engine_name, engine in searxng.engines.engines.items():
...
def fetch_wikidata_descriptions():
    searxng.network.set_timeout_for_thread(60)
...
def get_engine_descriptions_filename():
    return join(join(searxng_dir, "data"), "engine_descriptions.json")
