...
.. hint::
   ...
   To only update the traits of specific engines, you can run

     ./manage ENGINES="<engine_a> <engine_b>"
...
def main():
    """
    Load the engine traits and write them into :py:obj:`EngineTraitsMap.ENGINE_TRAITS_FILE`.

    Optionally, if arguments are provided via the command line, these are interpreted as the names
    of the engines that should be updated. All other engines will be left untouched.
    """
    engine_names_filter = None

    # if there are arguments provided (list of engine names), we only update the
    # engine traits for the specified engine names
    if len(sys.argv) > 1:
        engine_names_filter = sys.argv[1:]

    engines_cfg = []

    for eng_data in settings["engines"]:
        if not engine_names_filter or eng_data["name"] in engine_names_filter:
            eng_data["inactive"] = False
            engines_cfg.append(eng_data)

    load_engines(engines_cfg)
    # traits_map = EngineTraitsMap.from_data()
    traits_map = fetch_traits_map(engine_names_filter is not None)

    if engine_names_filter is None:
        sxng_tag_list = filter_locales(traits_map)
        write_languages_file(sxng_tag_list)


def fetch_traits_map(merge_with_existing_data: bool):
    ...
    traits_map.save_data(merge_with_existing_data)
    return traits_map
