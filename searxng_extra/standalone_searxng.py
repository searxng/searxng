...
... list(searxng.engines.categories.keys())
['general']
...
.. code::  bash
    ...
    $ python3 searxng_extra/standalone_searxng.py rain
...
... engine_cs = list(searxng.engines.categories.keys())
... # load module
... spec = importlib.util.spec_from_file_location(
...     'utils.standalone_searx', 'searxng_extra/standalone_searxng.py')
...
import searxng
import searxng.preferences
import searxng.query
import searxng.search
import searxng.webadapter
...
def get_search_query(
...
) -> searxng.search.SearchQuery:
    """Get  search results for the query"""
    if engine_categories is None:
        engine_categories = list(searxng.engines.categories.keys())
    try:
    ...
    preferences = searxng.preferences.Preferences(
        ['oscar'], engine_categories, searxng.engines.engines, [])
    preferences.key_value_settings['safesearch'].parse(args.safesearch)

    search_query = searxng.webadapter.get_search_query_from_webapp(
        preferences, form)[0]
    return search_query
...
def to_dict(search_query: searxng.search.SearchQuery) -> Dict[str, Any]:
    """Get result from parsed arguments."""
    result_container = searxng.search.Search(search_query).search()
...
def parse_argument(
    ...
    ... # load module
    ... spec = importlib.util.spec_from_file_location(
    ...     'utils.standalone_searx', 'utils/standalone_searxng.py')
    ... sas = importlib.util.module_from_spec(spec)
    ... spec.loader.exec_module(sas)
    ... sas.parse_argument()
    ...
    if not category_choices:
        category_choices = list(searxng.engines.categories.keys())
    parser = argparse.ArgumentParser(description='Standalone searxng.')
