...
class PluginStockChartTest(SearxTestCase):
    def setUp(self) -> None:
        ...
        self.storage.load_settings({
            "searx.plugins.stock_chart.SXNGPlugin": {"active": True}
        })
        self.storage.init(self.app)
        self.pref = searx.preferences.Preferences(
            ["simple"], ["general"], engines, self.storage
        )
    ...
    def test_normalize_accepts_identifiers(
    ...
    def test_timeframe_preference_and_default(
        self, user_timeframe: int | None, expected_days: int
    ) -> None:
    ...
    def test_api_returns_no_results(
