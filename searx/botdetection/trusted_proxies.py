...
from . import config
from ._helpers import _log_error_only_once, logger
...
class ProxyFix:
    ...
    def __call__(self, environ: WSGIEnvironment, start_response: StartResponse) -> abc.Iterable[bytes]:
        ...
        if not x_forwarded_for and not x_real_ip:
            _log_error_only_once("X-Forwarded-For nor X-Real-IP header is set!")

        if x_forwarded_for and not trusted_proxies:
            _log_error_only_once("missing botdetection.trusted_proxies config")
