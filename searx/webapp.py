...
def code_highlighter(codelines, filename=None, hl_lines=None, strip_whitespace=True, strip_new_lines=True):
    if not filename:
        filename = '.txt'

    try:
        lexer = get_lexer_for_filename(filename, stripall=strip_whitespace, stripnl=strip_new_lines)

    except Exception as e:  # pylint: disable=broad-except
        logger.warning("pygments lexer: %s " % e)
        # if lexer is not found, using default one
        lexer = get_lexer_for_filename('.txt', stripall=strip_whitespace, stripnl=strip_new_lines)
