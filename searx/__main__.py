...
class CustomTemplates(Jinja2Templates):
    ...
    def _create_env(self, directory: str) -> "jinja2.Environment":
        ...
        env.globals["url_for"] = CustomTemplates.url_for_theme
        env.install_gettext_callables(
                CustomTemplates.ugettext,
                CustomTemplates.ungettext,
                newstyle=True
        )
        return env
...
load_gettext_translations(directory="searx/translations", domain="messages")
templates = CustomTemplates(directory=settings['ui']['templates_path'])
routes = [
...
app = Starlette(routes=routes, debug=searx_debug, on_startup=[on_startup])
...
def format_date(date=None, format='medium', rebase=False):
    if rebase:
...
def code_highlighter(codelines, language=None):
    ...
    return html_code
templates.env.filters['code_highlighter'] = code_highlighter
...
def get_current_theme_name(request: Request, override: Optional[str] =None) -> str:
    ...
    if override and (override in themes or override == '__common__'):
        return override
    theme_name = request.query_params.get('theme', context.preferences.get_value('theme'))
...
async def search(request: Request):
    ...
    try:
        ...
        result_container = await loop.run_in_executor(None, search.search)
...
async def image_proxy(request: Request):
    ...
    try:
        ...
        if resp.status_code == 304:
            return PlainTextResponse('', status_code=resp.status_code)
        ...
        if resp.status_code != 200:
            ...
            if resp.status_code >= 400:
                return PlainTextResponse('Status code', status_code=resp.status_code)
        ...
        return StreamingResponse(forward_chunk(),
...
async def stats_checker(request: Request):
