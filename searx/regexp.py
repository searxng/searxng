...
OBJ_TUPLE = tuple[JSON, ...]
REGEXPR_LIST = list[str | tuple[str, OBJ_TUPLE]]
...
class RegExprList(abc.ABC):
    ...
    RE_GRP_PREFIX: str = "RegExprList"

    @abc.abstractmethod
    def load_regexps(self) -> REGEXPR_LIST:
    ...
    def __init__(self, chunk_size: int = 1000):
        ...
        self._chunks = None
        self._data_json: str | None = None

    def _get_data(self) -> REGEXPR_LIST:
    ...
    def get_chunks(self) -> list[tuple[re.Pattern, list[tuple]]]:
        ...
        """
        chunks = []
        re_list: REGEXPR_LIST = self._get_data()

        chunk_re = ""
        grp_tuples = []
        c = -1

        for pos in range(0, len(re_list)):  # pylint: disable=consider-using-enumerate
            c += 1
            objs_tpl: OBJ_TUPLE = ()
            if len(re_list[pos]) == 2:
                re_str, objs_tpl = re_list[pos]  # pyright: ignore[reportAssignmentType]
            else:
                re_str = re_list[pos]
            ...
            if m:
                ...
                for grp_name, val in m.groupdict().items():
                    ...
                    try:
                        ...
                        warnings.warn(
                            f"ignoring group '{grp_name}' in regexpr match {m}: check your regular expressions!"
                        )
                        m = None
                        break
        return None
...
def test_POC():
    test_list: REGEXPR_LIST = [
        ...
        (r'(.*\.)?academyirmbr\.com$', ("TEST string", "academyirmbr", 7, 8.2)),
        (r'(.*\.)?academyirm.*\.com$', ("TEST wildcard", "academyirm.*", 7, 8.2)),
