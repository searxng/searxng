...
def extract_text(
    xpath_results: list[ElementBase] | ElementBase | str | Number | bool | None, allow_none: bool = False
) -> str | None:
...
def eval_xpath(element: ElementBase, xpath_spec: XPathSpecType) -> XPath:
...
def eval_xpath_list(element: ElementBase, xpath_spec: XPathSpecType, min_len: int | None = None):
...
def eval_xpath_getindex(
    elements: ElementBase, xpath_spec: XPathSpecType, index: int, default: t.Any = _NOTSET
) -> t.Any:
    ...
    if -len(result) <= index < len(result):
        return result[index]  # pyright: ignore[reportUnknownVariableType]
    if default == _NOTSET:
