...
def _get_image_url(result: dict[str, t.Any], resolution: str) -> str:
    return f"{cdn_url}/{result['platform']}/{resolution}/{result['commonName']}.jpg"
...
def response(resp: "SXNG_Response"):
    ...
    for result in icons:
        ...
        if result["id"] in svgs:
            thumbnail_url = svgs[result["id"]]
            img_url = svgs[result["id"]]
        else:
            # any very big resolution will give us the max-res image
            img_url = _get_image_url(result, "10000")
            thumbnail_url = _get_image_url(result, "512")
            ...
            res.types.LegacyResult(
                {
                    ...
                    "img_src": img_url,
                    "title": result["name"],
                    "content": result.get("subcategory"),
                    "img_format": result["sourceFormat"],
                }
            )
        )
