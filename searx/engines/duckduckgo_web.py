...
This implementation fetches the link to the first API page (i.e. ``links.duckduckgo.com/d.js?...``) from duckduckgo.com
and uses the ``n`` parameter of the API to fetch all subsequent pages.

This also means that it's not possible to immediately search for the first
...
def _fetch_first_page_link(
    ...
    """
    Search for a ``<link id="deep_preload_link" rel="preload" as="script" href="https://links.duckduckgo.com/d.js?q=rust&t=D&l=us-en&s=0&a=h_&ct=DE&vqd=VQD_VALUE&bing_market=en-US&p_ent=&ex=-1&dp=LONG_TOKEN>``. This points to the first page
    ...
    CACHE.set(cache_key, first_page_link)
