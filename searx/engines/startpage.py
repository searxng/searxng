...
from searx.utils import extr, extract_text, eval_xpath, gen_useragent, html_to_text, humanize_bytes
...
def _parse_published_date(content: str) -> tuple[str, Optional[str]]:
    published_date = None
    ...
    for results_categ in results_obj.get('mainline', []):
        for item in results_categ.get('results', []):
            if results_categ['display_type'] == 'web-google':
                ...
                results.append(_get_image_result(item))
