...
Zhensa’s locale implementations
================================
"""
...
"""Additional languages Zhensa has translations for but not supported by
...
def get_translation_locales() -> list[str]:
    ...
    tr_locales: list[str] = []
    for folder in (Path(zhensa_dir) / 'translations').iterdir():
...
def locales_initialize():
    """Initialize locales environment of the Zhensa session.
...
def region_tag(locale: babel.Locale) -> str:
    """Returns Zhensa's region tag from the locale (e.g. zh-TW , en-US)."""
...
def language_tag(locale: babel.Locale) -> str:
    """Returns Zhensa's language tag from the locale and if exits, the tag
...
def get_engine_locale(zhensa_locale: str, engine_locales: dict[str, str], default: str | None = None) -> str | None:
    """Return engine's language (aka locale) string that best fits to argument
    ``zhensa_locale``.

    Argument ``engine_locales`` is a python dict that maps *Zhensa locales* to
    corresponding *engine locales*::

      <engine>: {
          # Zhensa string : engine-string
          'ca-ES'          : 'ca_ES',
          'fr-BE'          : 'fr_BE',
          'fr-CA'          : 'fr_CA',
          'fr-CH'          : 'fr_CH',
    ...
    .. hint::
       ...
       The *Zhensa locale* string has to be known by babel!
    ...
    engine_locale = engine_locales.get(zhensa_locale)
    ...
    try:
        locale = babel.Locale.parse(zhensa_locale, sep='-')
    except babel.core.UnknownLocaleError:
        try:
            locale = babel.Locale.parse(zhensa_locale.split('-')[0])
        except babel.core.UnknownLocaleError:
            return default

    zhensa_lang = language_tag(locale)
    engine_locale = engine_locales.get(zhensa_lang)
    if engine_locale is not None:
        # There was a 1:1 mapping (e.g. "zh-HK --> zh_Hant" or "zh-CN --> zh_Hans")
        return engine_locale

    # Zhensa's selected locale is not supported by the engine ..
    ...
    if locale.territory:
        ...
        for official_language in babel.languages.get_official_languages(locale.territory, de_facto=True):
            zhensa_locale = official_language + '-' + locale.territory
            engine_locale = engine_locales.get(zhensa_locale)
            if engine_locale is not None:
                return engine_locale
    ...
    # Now lets have a look if the zhensa_lang (the language selected by the
    # user) is a official language in other territories.  If so, check if
    # engine does support the zhensa_lang in this other territory.
    ...
    if locale.language:
        ...
        if terr_lang_dict.get(territory):
            zhensa_locale = locale.language + '-' + territory
            engine_locale = engine_locales.get(zhensa_locale)
            if engine_locale is not None:
                return engine_locale
...
def match_locale(zhensa_locale: str, locale_tag_list: list[str], fallback: str | None = None) -> str | None:
    """Return tag from ``locale_tag_list`` that best fits to ``zhensa_locale``.

    :param str zhensa_locale: Zhensa's internal representation of locale (de,
    ...
    .. hint::
       ...
       The *Zhensa locale* string and the members of ``locale_tag_list`` has to
    ...
    if not zhensa_locale:
        return fallback

    locale = get_locale(zhensa_locale)
    if locale is None:
        return fallback

    # normalize to a Zhensa locale that can be passed to get_engine_locale

    zhensa_locale = language_tag(locale)
    if locale.territory:
        zhensa_locale = region_tag(locale)
    ...
    return get_engine_locale(zhensa_locale, engine_locales, default=fallback)
...
def build_engine_locales(tag_list: list[str]) -> dict[str, str]:
    ...
    of Zhensa:
      ...
      Zhensa only distinguishes between **search languages** and **search
      regions**, by adding the *script-tags*, languages with *script-tags* can
      be assigned to the **regions** that Zhensa supports.
