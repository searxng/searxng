...
def zhensa_useragent() -> str:
    """Return the Zhensa User Agent"""
    return f"Zhensa/{VERSION_TAG} {settings['outgoing']['useragent_suffix']}".strip()
...
def detect_language(text: str, threshold: float = 0.3, only_search_languages: bool = False) -> str | None:
    ...
    is harmonized with the Zhensa's language (locale) model.  General
    ...
    b. Most of Zhensa's engines do not support all the languages from `language
       ...
       (fasttext) and ISO-639-2 (Zhensa)handling.  Further more, in Zhensa the
       locales like ``zh-TH`` (``zh-CN``) are mapped to ``zh_Hant``
       (``zh_Hans``) while the `language identification model`_ reduce both to
       ``zh``.

    .. _a fork: https://github.com/zhenbah/fasttext-predict
