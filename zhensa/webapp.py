...
from zhensa import limiter
from zhensa.botdetection import link_token, ProxyFix

from zhensa.data import ENGINE_DESCRIPTIONS
...
from zhensa.webadapter import (
    get_search_query_from_webapp,
    get_selected_categories,
    parse_lang,
)
from zhensa.utils import gen_useragent, dict_subset
from zhensa.version import VERSION_STRING, GIT_URL, GIT_BRANCH
from zhensa.query import RawTextQuery
from zhensa.plugins.oa_doi_rewrite import get_doi_resolver
...
from zhensa.metrics import get_engines_stats, get_engine_errors, get_reliabilities, histogram, counter, openmetrics
from zhensa.flaskfix import patch_application
...
# renaming names from zhensa imports ...
from zhensa.autocomplete import search_autocomplete, backends as autocomplete_backends
from zhensa import favicons

from zhensa.valkeydb import initialize as valkey_initialize
from zhensa.sxng_locales import sxng_locales
import zhensa.search
from zhensa.network import stream as http_stream, set_context_network_name
from zhensa.search.checker import get_result as checker_get_result
...
def pre_request():
    ...
    preferences = Preferences(themes, list(categories.keys()), engines, zhensa.plugins.STORAGE, client_pref)
    ...
    for plugin in zhensa.plugins.STORAGE:
...
def index_error(output_format: str, error_message: str):
    ...
    if output_format == 'csv':
        ...
        cont_disp = 'attachment;Filename=zhensa.csv'
...
def autocompleter():
    ...
    for obj in zhensa.answerers.STORAGE.ask(sug_prefix):
    ...
    if len(raw_text_query.autocomplete_list) == 0 and len(sug_prefix) > 0:
        ...
        # get Zhensa's locale and autocomplete backend from cookie
...
def preferences():
    ...
    return render(
        ...
        supports = supports,
        answer_storage = zhensa.answerers.STORAGE.info,
        disabled_engines = disabled_engines,
        autocomplete_backends = autocomplete_backends,
        favicon_resolver_names = favicons.proxy.CFG.resolver_map.keys(),
        shortcuts = {y: x for x, y in engine_shortcuts.items()},
        themes = themes,
        plugins_storage = zhensa.plugins.STORAGE.info,
...
def image_proxy():
    ...
    try:
        ...
        if resp and not forward_resp:
            ...
            # we make sure to close the response between zhensa and the HTTP server
            try:
                resp.close()
            except httpx.HTTPError:
...
def engine_descriptions():
    ...
    for engine_name, engine_mod in engines.items():
        ...
        if descr is not None:
            result[engine_name] = [descr, "Zhensa config"]
...
def config():
    ...
    _plugins = []
    for _ in zhensa.plugins.STORAGE:
    ...
    return jsonify(
        {
            ...
            'limiter': {
                ...
                'botdetection.ip_lists.pass_zhensa_org': _limiter_cfg.get('botdetection.ip_lists.pass_zhensa_org'),
            },
...
def run():
    ...
    This run method is only called when Zhensa is started via ``__main__``::

        python -m zhensa.webapp
    ...
    command line script’s run support::
        ...
        flask --app zhensa.webapp run --debug --reload --host 127.0.0.1 --port 8888
...
def init():
    ...
    if zhensa.sxng_debug or app.debug:
        app.debug = True
        zhensa.sxng_debug = True
    ...
    locales_initialize()
    valkey_initialize()
    zhensa.plugins.initialize(app)

    metrics: bool = get_setting("general.enable_metrics")  # type: ignore
    zhensa.search.initialize(enable_checker=True, check_network=True, enable_metrics=metrics)
