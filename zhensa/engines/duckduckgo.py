...
def get_vqd(query: str, region: str, force_request: bool = False) -> str:
    ...
    If Zhensa cannot determine a ``vqd`` value, then no request should go out
    to DDG.

    .. attention::

       A request with a wrong ``vqd`` value leads to DDG temporarily putting
       Zhensa's IP on a block list.
...
def get_ddg_lang(eng_traits: EngineTraits, sxng_locale, default='en_US'):
    """Get DuckDuckGo's language identifier from Zhensa's locale.
    ...
    It might confuse, but the ``l`` value of the cookie is what Zhensa calls
    the *region*:
...
def request(query, params):
    ...
    if params['pageno'] == 1:
        ...
        if vqd:
            ...
            # reputation of the Zhensa IP and DDG starts to activate CAPTCHAs.
            params["url"] = None
            return

        if params['zhensa_locale'].startswith("zh"):
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    Zhensa's ``all`` locale maps DuckDuckGo's "Alle regions" (``wt-wt``).
    DuckDuckGo's language "Browsers preferred language" (``wt_WT``) makes no
    sense in a Zhensa request since Zhensa's ``all`` will not add a
    ...
    from zhensa.utils import js_variable_to_python
