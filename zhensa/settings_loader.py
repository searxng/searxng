...
- By default, local configurations are expected in folder ``/etc/zhensa`` from
  where applications can load them with the :py:obj:`get_yaml_cfg` function.

- By default, customized :ref:`Zhensa appl <zhensa settings.yml>` settings are
...
zhensa_dir = os.path.abspath(os.path.dirname(__file__))
...
def get_user_cfg_folder() -> Path | None:
    ...
    1. If the ``ZHENSA_SETTINGS_PATH`` environment is set and points to a
       folder (e.g. ``/etc/mysxng/``), all local configurations are expected in
       this folder.  The settings of the :ref:`Zhensa appl <zhensa
       settings.yml>` then expected in ``settings.yml``
       (e.g. ``/etc/mysxng/settings.yml``).

    2. If the ``ZHENSA_SETTINGS_PATH`` environment is set and points to a file
       (e.g. ``/etc/mysxng/myinstance.yml``), this file contains the settings of
       the :ref:`Zhensa appl <zhensa settings.yml>` and the folder
       (e.g. ``/etc/mysxng/``) is used for all other configurations.

       This type (``ZHENSA_SETTINGS_PATH`` points to a file) is suitable for
       use cases in which different profiles of the :ref:`Zhensa appl <zhensa
       settings.yml>` are to be managed, such as in test scenarios.

    3. If folder ``/etc/zhensa`` exists, it is used.

    In case none of the above path exists, ``None`` is returned.  In case of
    environment ``ZHENSA_SETTINGS_PATH`` is set, but the (folder or file) does
    not exists, a :py:obj:`EnvironmentError` is raised.

    """

    folder = None
    settings_path = os.environ.get("ZHENSA_SETTINGS_PATH")

    # Disable default /etc/zhensa is intended exclusively for internal testing purposes
    # and is therefore not documented!
    disable_etc = os.environ.get('ZHENSA_DISABLE_ETC_SETTINGS', '').lower() in ('1', 'true')
...
def load_settings(load_user_settings: bool = True) -> tuple[SettingsType, str]:
    """Function for loading the settings of the Zhensa application
    (:ref:`settings.yml <zhensa settings.yml>`)."""
    ...
    settings_yml = os.environ.get("ZHENSA_SETTINGS_PATH")
    if settings_yml and Path(settings_yml).is_file():
        # see get_user_cfg_folder() --> ZHENSA_SETTINGS_PATH points to a file
        settings_yml = Path(settings_yml).name
    else:
        # see get_user_cfg_folder() --> ZHENSA_SETTINGS_PATH points to a folder
