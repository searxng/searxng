...
from zhensa import logger as log
import zhensa.engines
from zhensa.metrics import histogram_observe, counter_add
from zhensa.result_types import Result, LegacyResult, MainResult
from zhensa.result_types.answer import AnswerSet, BaseAnswer
...
def calculate_score(
    ...
    for result_engine in result['engines']:
        if hasattr(zhensa.engines.engines.get(result_engine), 'weight'):
            weight *= float(zhensa.engines.engines[result_engine].weight)
...
class ResultContainer:
    ...
    def extend(
        ...
        if engine_name in zhensa.engines.engines:
            eng = zhensa.engines.engines[engine_name]
    ...
    def get_ordered_results(self) -> list[MainResult | LegacyResult]:
        ...
        for res in results:
            ...
            engine = zhensa.engines.engines.get(res.engine or "")
            if engine:
    ...
    def add_unresponsive_engine(self, engine_name: str, error_type: str, suspended: bool = False):
        with self._lock:
            if self._closed:
                ...
                return
            if zhensa.engines.engines[engine_name].display_error_messages:
...
def merge_two_infoboxes(origin: LegacyResult, other: LegacyResult):
    ...
    # pylint: disable=too-many-branches
    weight1 = getattr(zhensa.engines.engines[origin.engine], "weight", 1)
    weight2 = getattr(zhensa.engines.engines[other.engine], "weight", 1)
