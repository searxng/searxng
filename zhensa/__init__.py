...
import zhensa.unixthreadname  # pylint: disable=unused-import
...
zhensa_dir: str = abspath(dirname(__file__))
zhensa_parent_dir: str = abspath(dirname(dirname(__file__)))
...
def init_settings():
    ...
    ``logger`` from ``ZHENSA_SETTINGS_PATH``.
    """

    # pylint: disable=import-outside-toplevel
    from zhensa import settings_loader
    from zhensa.settings_defaults import SCHEMA, apply_schema
...
def _logging_config_debug():
    ...
    log_level = os.environ.get('ZHENSA_DEBUG_LOG_LEVEL', 'DEBUG')
