...
SCHEMA: dict[str, t.Any] = {
    'general': {
        'debug': SettingsValue(bool, False, 'ZHENSA_DEBUG'),
        'instance_name': SettingsValue(str, 'Zhensa'),
        'privacypolicy_url': SettingsValue((None, False, str), None),
        'contact_url': SettingsValue((None, False, str), None),
        'donation_url': SettingsValue((bool, str), "https://docs.zhensa.org/donate.html"),
        'enable_metrics': SettingsValue(bool, True),
        'open_metrics': SettingsValue(str, ''),
    },
    'brand': {
        'issue_url': SettingsValue(str, 'https://github.com/zhenbah/zhensa/issues'),
        'new_issue_url': SettingsValue(str, 'https://github.com/zhenbah/zhensa/issues/new'),
        'docs_url': SettingsValue(str, 'https://docs.zhensa.org'),
        'public_instances': SettingsValue((False, str), 'https://zhensa.space'),
        'wiki_url': SettingsValue((False, str), 'https://github.com/zhenbah/zhensa/wiki'),
    ...
    'server': {
        'port': SettingsValue((int, str), 8888, 'ZHENSA_PORT'),
        'bind_address': SettingsValue(str, '127.0.0.1', 'ZHENSA_BIND_ADDRESS'),
        'limiter': SettingsValue(bool, False, 'ZHENSA_LIMITER'),
        'public_instance': SettingsValue(bool, False, 'ZHENSA_PUBLIC_INSTANCE'),
        'secret_key': SettingsValue(str, environ_name='ZHENSA_SECRET'),
        'base_url': SettingsValue((False, str), False, 'ZHENSA_BASE_URL'),
        'image_proxy': SettingsValue(bool, False, 'ZHENSA_IMAGE_PROXY'),
        'http_protocol_version': SettingsValue(('1.0', '1.1'), '1.0'),
        'method': SettingsValue(('POST', 'GET'), 'POST', 'ZHENSA_METHOD'),
        'default_http_headers': SettingsValue(dict, {}),
    },
    # redis is deprecated ..
    'redis': {
        'url': SettingsValue((None, False, str), False, 'ZHENSA_REDIS_URL'),
    },
    'valkey': {
        'url': SettingsValue((None, False, str), False, 'ZHENSA_VALKEY_URL'),
    },
    'ui': {
        'static_path': SettingsDirectoryValue(str, os.path.join(zhensa_dir, 'static')),
        'templates_path': SettingsDirectoryValue(str, os.path.join(zhensa_dir, 'templates')),
