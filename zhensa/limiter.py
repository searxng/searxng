...
itself.  As a result, the Zhensa engine then receives a CAPTCHA or is blocked
by the search engine (the origin) in some other way.

To avoid blocking, the requests from bots to Zhensa must also be blocked, this
...
.. _NGINX RequestHeader:
    https://docs.zhensa.org/admin/installation-nginx.html#nginx-s-zhensa-site
.. _Apache RequestHeader:
    https://docs.zhensa.org/admin/installation-apache.html#apache-s-zhensa-site
...
file ``/etc/zhensa/limiter.toml``.  The defaults are shown in limiter.toml_ /
...
import zhensa.compat
from zhensa import (
    logger,
    valkeydb,
)
from zhensa import botdetection
from zhensa.extended_types import SXNG_Request, sxng_request
from zhensa.botdetection import (
    config,
    http_accept,
...
def get_cfg() -> config.Config:
    """Returns Zhensa's global limiter configuration."""
    global CFG  # pylint: disable=global-statement

    if CFG is None:
        from . import settings_loader  # pylint: disable=import-outside-toplevel

        cfg_file = (settings_loader.get_user_cfg_folder() or Path("/etc/zhensa")) / "limiter.toml"
        CFG = config.Config.from_toml(LIMITER_CFG_SCHEMA, cfg_file, zhensa.compat.LIMITER_CFG_DEPRECATED)
        zhensa.compat.limiter_fix_cfg(CFG, cfg_file)
...
def initialize(app: flask.Flask, settings):
    ...
    if not valkey_client:
        logger.error(
            ...
            "https://docs.zhensa.org/admin/zhensa.limiter.html"
        )
