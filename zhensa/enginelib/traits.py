...
from zhensa.data import data_dir, ENGINE_TRAITS
...
class EngineTraits:
    ...
    """Maps Zhensa's internal representation of a region to the one of the engine.

    Zhensa's internal representation can be parsed by babel and the value is
    send to the engine:
    ...
    .. code:: python
       ...
       for key, egnine_region regions.items():
          zhensa_region = babel.Locale.parse(key, sep='-')
          ...
    """

    languages: dict[str, str] = dataclasses.field(default_factory=dict)
    """Maps Zhensa's internal representation of a language to the one of the engine.
    ...
    .. code:: python
       ...
       for key, egnine_lang in languages.items():
          zhensa_lang = babel.Locale.parse(key)
          ...
    """

    all_locale: str | None = None
    """To which locale value Zhensa's ``all`` language is mapped (shown a "Default
    language").
    """
    ...
    """A place to store engine's custom traits, not related to the Zhensa core.
    """

    def get_language(self, zhensa_locale: str, default: t.Any = None):
        """Return engine's language string that *best fits* to Zhensa's locale.

        :param zhensa_locale: Zhensa's internal representation of locale
        ...
        return locales.get_engine_locale(zhensa_locale, self.languages, default=default)

    def get_region(self, zhensa_locale: str, default: t.Any = None) -> t.Any:
        """Return engine's region string that best fits to Zhensa's locale.
        ...
        if zhensa_locale == 'all' and self.all_locale is not None:
            return self.all_locale
        return locales.get_engine_locale(zhensa_locale, self.regions, default=default)

    def is_locale_supported(self, zhensa_locale: str) -> bool:
        """A *locale* (Zhensa's internal representation) is considered to be
        ...
        if self.data_type == 'traits_v1':
            return bool(self.get_region(zhensa_locale) or self.get_language(zhensa_locale))
...
class EngineTraitsMap(dict[str, EngineTraits]):
    ...
    def fetch_traits(cls, log: t.Callable[[str], None]) -> 'EngineTraitsMap':
        from zhensa import engines  # pylint: disable=cyclic-import, import-outside-toplevel
        ...
        for engine_name in names:
            ...
            if traits is not None:
                log("%-20s: Zhensa languages --> %s " % (engine_name, len(traits.languages)))
                log("%-20s: Zhensa regions   --> %s" % (engine_name, len(traits.regions)))
