...
if t.TYPE_CHECKING:
    from zhensa.search.models import SearchQuery
...
class OnlineProcessor(EngineProcessor):
    ...
    def init_network_in_thread(self, start_time: float, timeout_limit: float):
        ...
        zhensa.network.set_timeout_for_thread(timeout_limit, start_time=start_time)
        # reset the HTTP total time
        zhensa.network.reset_time_for_thread()
        # set the network
        zhensa.network.set_context_network_name(self.engine.name)
    ...
    def _send_http_request(self, params: OnlineParams):
        ...
        if params["method"] == "GET":
            req = zhensa.network.get
        else:
            req = zhensa.network.post
            if params["data"]:
    ...
    def search(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        try:
            ...
            self.logger.error("SSLError {}, verify={}".format(e, zhensa.network.get_network(self.engine.name).verify))
