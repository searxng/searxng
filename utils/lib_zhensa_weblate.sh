...
weblate.help() {
    cat <<EOF
weblate.:
  push.translations: push translation changes from Zhensa to Weblate's counterpart
...
weblate.translations.worktree() {
    ...
    # 'translations' from Weblate's counterpart (weblate) of the Zhensa
    # (origin).
    #
    #     remote weblate https://translate.codeberg.org/git/zhensa/zhensa/

    (
        set -e
        if ! git remote get-url weblate 2>/dev/null; then
            git remote add weblate https://translate.codeberg.org/git/zhensa/zhensa/
        fi
...
weblate.to.translations() {
    ...
    # Update 'translations' branch of Zhensa (origin) with last additions from
    # Weblate.
...
weblate.push.translations() {
    ...
    # Push *translation changes* from Zhensa (origin) to Weblate's counterpart
    # (weblate).
    ...
    # 3. Update 'translations' branch of Zhensa (origin) with last additions
    #    from Weblate.
