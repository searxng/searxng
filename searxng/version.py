...
# fallback values
# if there is searxng.version_frozen module, and it is not possible to get the git tag
...
def get_git_version():
    ...
    # add "-dirty" suffix if there are uncommited changes except searxng/settings.yml
    try:
        subprocess_run(
            "git diff --quiet -- . ':!searxng/settings.yml' ':!utils/brand.env'"
        )
...
try:
    from searxng.version_frozen import VERSION_STRING, VERSION_TAG, GIT_URL, GIT_BRANCH
except ImportError:
    try:
        try:
...
# this file is generated automatically by searxng/version.py
...
"""
        ...
        # usage: eval "$(python -m searxng.version)"
        shell_code = f"""
VERSION_STRING="{VERSION_STRING}"
VERSION_TAG="{VERSION_TAG}"
GIT_URL="{GIT_URL}"
