...
from searxng.data import ENGINE_DESCRIPTIONS
from searxng.settings_defaults import OUTPUT_FORMATS
from searxng.settings_loader import get_default_settings_path
from searxng.exceptions import SearxParameterException
from searxng.engines import (
    categories,
    engines,
    engine_shortcuts,
)
from searxng.webutils import (
    UnicodeWriter,
    highlight_content,
    get_static_files,
...
from searxng.webadapter import (
    get_search_query_from_webapp,
    get_selected_categories,
)
from searxng.utils import (
    html_to_text,
    gen_useragent,
    dict_subset,
    match_language,
...
from searxng.plugins import plugins
from searxng.plugins.oa_doi_rewrite import get_doi_resolver
from searxng.preferences import (
    Preferences,
    ValidationException,
    LANGUAGE_CODES,
)
from searxng.answerers import (
    answerers,
    ask,
)
from searxng.metrics import (
    get_engines_stats,
    get_engine_errors,
    get_reliabilities,
    histogram,
...
from searxng.flaskfix import patch_application

# renaming names from searxng imports ...

from searxng.autocomplete import search_autocomplete, backends as autocomplete_backends
from searxng.languages import language_codes as languages
from searxng.locales import LOCALE_NAMES, UI_LOCALE_CODES, RTL_LOCALES
from searxng.search import SearchWithPlugins, initialize as search_initialize
from searxng.network import stream as http_stream, set_context_network_name
from searxng.search.checker import get_result as checker_get_result

logger = logger.getChild('webapp')

# check secret_key
if not searxng_debug and settings['server']['secret_key'] == 'ultrasecretkey':
...
# True if "FLASK_APP=searxng/webapp.py FLASK_ENV=development flask run"
...
#  __name__ != "__main__" if searxng.webapp is imported (make test, make docs, uwsgi...)
# see run() at the end of this file : searxng_debug activates the reload feature.
werkzeug_reloader = flask_run_development or (searxng_debug and __name__ == "__main__")
...
exception_classname_to_text = {
    ...
    'searxng.exceptions.SearxEngineCaptchaException': gettext("CAPTCHA"),
    'searxng.exceptions.SearxEngineTooManyRequestsException': gettext("too many requests"),
    'searxng.exceptions.SearxEngineAccessDeniedException': gettext("access denied"),
    'searxng.exceptions.SearxEngineAPIException': gettext("server API error"),
    'searxng.exceptions.SearxEngineXPathException': parsing_error_text,
...
def render(template_name, override_theme=None, **kwargs):
    ...
    # i18n
    kwargs['language_codes'] = languages  # from searxng.languages
...
def index_error(output_format, error_message):
    ...
    if output_format == 'csv':
        ...
        cont_disp = 'attachment;Filename=searxng.csv'
...
def search():
    ...
    if output_format == 'csv':
        ...
        cont_disp = 'attachment;Filename=searxng_-_{0}.csv'.format(search_query.query)
        response.headers.add('Content-Disposition', cont_disp)
        return response
...
def autocompleter():
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        # the suggestion request comes from the searxng search form
