...
import searxng.search.checker
from searxng.search import PROCESSORS
from searxng.engines import engine_shortcuts
...
def iter_processor(engine_name_list):
    if len(engine_name_list) > 0:
        ...
        for name, processor in searxng.search.PROCESSORS.items():
            yield name, processor
...
def run(engine_name_list, verbose):
    ...
    for name, processor in iter_processor(engine_name_list):
        ...
        checker = searxng.search.checker.Checker(processor)
        checker.run()
