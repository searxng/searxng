...
from searxng.metrics.error_recorder import count_error
...
class OnlineProcessor(EngineProcessor):
    ...
    def initialize(self):
        ...
        searxng.network.set_timeout_for_thread(self.engine.timeout, start_time=default_timer())
        # reset the HTTP total time
        searxng.network.reset_time_for_thread()
        # set the network
        searxng.network.set_context_network_name(self.engine_name)
        super().initialize()
    ...
    def _send_http_request(self, params):
        ...
        if params['method'] == 'GET':
            req = searxng.network.get
        else:
            req = searxng.network.post
    ...
    def search(self, query, params, result_container, start_time, timeout_limit):
        # set timeout for all HTTP requests
        searxng.network.set_timeout_for_thread(timeout_limit, start_time=start_time)
        # reset the HTTP total time
        searxng.network.reset_time_for_thread()
        # set the network
        searxng.network.set_context_network_name(self.engine_name)
