...
from searxng.data import USER_AGENTS
from searxng.version import VERSION_TAG
from searxng.languages import language_codes
...
def searxng_useragent():
    """Return the searxng User Agent"""
    return 'searxng/{searxng_version} {suffix}'.format(
           searxng_version=VERSION_TAG,
...
def gen_useragent(os=None):
    ...
    See searxng/data/useragents.json
    """
...
def extract_url(xpath_results, base_url):
    ...
    Example:
        >>> def f(s, search_url):
        >>>    return searxng.utils.extract_url(html.fromstring(s), search_url)
        ...
        >>> searxng.utils.extract_url([], 'https://example.com')
        raise ValueError
...
def is_valid_lang(lang):
    ...
    Examples:
        ...
        >>> is_valid_lang('en')
        (True, 'en', 'english')
        >>> searxng.utils.is_valid_lang('Español')
        (True, 'es', 'spanish')
        >>> searxng.utils.is_valid_lang('Spanish')
        (True, 'es', 'spanish')
    """
