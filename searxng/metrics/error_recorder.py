...
def get_trace(traces):
    for trace in reversed(traces):
        ...
        if '/'.join(split_filename[-3:-1]) == 'searxng/engines':
            return trace
        if '/'.join(split_filename[-4:-1]) == 'searxng/search/processors':
            return trace
    return traces[-1]
...
def get_error_context(framerecords, exception_classname, log_message, log_parameters, secondary) -> ErrorContext:
    searxng_frame = get_trace(framerecords)
    filename = searxng_frame.filename
    if filename.startswith(searxng_parent_dir):
        filename = filename[len(searxng_parent_dir) + 1:]
    function = searxng_frame.function
    line_no = searxng_frame.lineno
    code = searxng_frame.code_context[0].strip()
    del framerecords
